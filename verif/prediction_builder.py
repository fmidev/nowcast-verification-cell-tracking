...
Classes are instanciated using a configuration Dict.

Depends on addict, numpy, h5py, pysteps
...
class PredictionBuilder(ABC):
    ...
    def __init__(self, config: Dict):
        ...
        Args:
            config (Dict): Attribute dictionary defined in
    ...
    def run(self, timestamp: str):
        data = self.read_input(timestamp, **self.input_params)
        ...
        nowcast = self.postprocessing(
            nowcast=nowcast, metadata=metadata, params=self.postprocessing_params
        )
        return nowcast
    ...
    def read_input(self, timestamp: str, num_next_files: int = 0):
        ...
        Z, _, metadata = io.read_timeseries(fns, self.importer, **self.importer_kwargs)
    ...
    @abstractmethod
    def postprocessing(self, data, metadata, params):
        """
