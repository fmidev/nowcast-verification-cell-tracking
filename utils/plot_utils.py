...
def _get_colormap(quantity):
    ...
    if quantity == "HCLASS":
        ...
        bounds = np.arange(QTY_RANGES[quantity][0], QTY_RANGES[quantity][1] + 0.01, 0.25)
        norm = mpl.colors.BoundaryNorm(boundaries=bounds, ncolors=len(bounds))
        ...
        norm = mpl.colors.TwoSlopeNorm(vmin=QTY_RANGES[quantity][0], vcenter=3, vmax=QTY_RANGES[quantity][1])
